...
import styles from "./DepApproveAndReject.module.css";
...
const ReceivedandQueryUploadedonPortal = ({
  newCaseData = {},
  status = "",
  action = "",
}: ApproveModalProps) => {
  ...
  const uploadFile = async () => {
    ...
    const enhanceApi = {
      Received: "Queryreceivedaudittrail",
      'Query Uploaded on Portal': "Querysubmittedaudittrail",
    };
    const fciApi = {
    ...
    const URLFORMCREATIONAUDITTRIAL = `/${
      //@ts-ignore
      action === "Query" ? enhanceApi[status] : fciApi[status]}?email=${user}&casenumber=${newCaseData?.caseNumber}`;
...
export default ReceivedandQueryUploadedonPortal;
